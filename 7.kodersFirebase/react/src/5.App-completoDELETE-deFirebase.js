...
const deleteKoder = async koderKey => {
   ...
   let result = await fetch(`https://react2022-5a405-default-rtdb.firebaseio.com/koders/${koderKey}.json`, {
      method: "DELETE"
   })
   result = await result.json()
   ...
   {/*
   cada vez que Borras un koder,
   haces el fetch de la lista de firebase otra vez,
   actualiza la lista, y CONFIRMA que se borró al no desplegarse
   */}
                              ...
                              <div className="card">
                                 <div className="card-body">
                                    ...
                                    <p className="card-text">Generación: {generation}</p>
                                       <button className="btn btn-danger" onClick={() => deleteKoder(koderKey)}>Borrar koder</button>
                                 </div>
                              </div>
                           </div>
                        )
